...
import RotatingLines from '../components/RotatingLines.js';

async function checkSecurity() {
  await new Promise((resolve) => setTimeout(resolve, 2000));
}

const Wrapper = styled.div<{ isPending: boolean; isSuccess: boolean; showText: boolean }>(
  ({ isPending, isSuccess, showText }) => ({
    display: 'inline-flex',
    ...
    borderRadius: '9999px',
    backgroundColor: isPending ? 'transparent' : isSuccess ? 'var(--kzero-success-color)' : 'var(--kzero-error-color)',
    color: isPending ? 'var(--kzero-text-color)' : '#FFFFFF',
    whiteSpace: 'nowrap',
    overflow: 'hidden',
    maxWidth: showText ? '100%' : 26,
    minWidth: 26,
    '& > svg': {
      flex: '0 0 auto'
    }
...
function SecurityCheck() {
  ...
  return (
    <Wrapper isPending={status === 'pending'} showText={showText} isSuccess={status === 'success'}>
      {status === 'pending' && (
        <>
