...
import { AddressCell } from '@kzero/zk-react';
...
function Authed({
  ...
  const [api] = useState<ApiPromise>(
    () =>
      new ApiPromise({
        initWasm: true,
        provider: new WsProvider('wss://node-template.kzero.xyz')
      })
  );
    ...
    toast.promise(
      async () => {
        ...
        return new Promise<string>(async (resolve, reject) => {
          try {
            const injected = await window.injectedWeb3?.['kzero'].enable(window.location.origin);
            ...
            const results = await signer.signPayload?.({ address, method: transfer.method.toHex() } as any);

            if (!results?.signedTransaction) return;

            api.tx(results.signedTransaction).send((results) => {
              if (results.status.isInBlock) {
                resolve(results.status.asInBlock.toString());
            ...
            <Box>
              ...
              <Typography>
                <Link
                  ...
                  target='_blank'
                  href={`https://polkadot.js.org/apps/?rpc=wss://node-template.kzero.xyz#/explorer/query/${props.data}`}
                >
                  View on Polkadot.js
                </Link>
              </Typography>
